document.addEventListener('DOMContentLoaded', () => {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        // Manually handle consent checkbox
        const consentCheckbox = document.getElementById('consent');
        if (consentCheckbox) {
            formData.append('consent', consentCheckbox.checked ? 'Accepted' : 'Not Accepted');
        }
        ...
        if (audioChunks.length > 0) {
            const audioBlob = new Blob(audioChunks, { type: 'audio/wav' });
            // Using 'attachment' as name which many services recognize, or just 'audio'
            formData.append('attachment', audioBlob, 'voice_memo.wav');
        }
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.log('Email successfully sent via FormSubmit.');
            } else {
                throw new Error('Server returned ' + response.status);
            }
        } catch (error) {
            console.error('Error sending form:', error);
            alert('There was a problem sending your answer. If this is the first time you are using this form, please check your email (primarnoplava@gmail.com) to activate it.\n\nError: ' + error.message);
